unit Stock_Property_Grid;
{В данной таблице отображаются свойства акции }
...
type
  ...
  { tStockGrid }
  tStockPropertyGrid = class (TStringGrid)
   private
     fStock : IStock;
   public
...
constructor tStockPropertyGrid.Create(AOwner: TComponent);
begin
  ...
  self.Cells[0,6]:='Портфель';
end;

procedure tStockPropertyGrid.updateData(stock : IStock);
...
begin
  ...
  self.Cells[1,6]:=stock.portfolios.CommaText;
end;

procedure tStockPropertyGrid.OnGridDrawCell(Sender: TObject; aCol, aRow: Integer;
